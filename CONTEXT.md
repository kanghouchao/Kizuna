...
受注 1 件が会員へ帰属した事実の受注単位の記録（根拠・帰属時刻を持つ）。帰属は**完了の瞬間**（完了時の関連解決 — 根拠 COMPLETION）または**事後申領の瞬間**（伝票トークン — 根拠 RECEIPT_TOKEN）にだけ生まれ、成立後は関連の状態に影響されない**不変の事実**である（ADR 0009）。会員の来店履歴はこの記録から読む（関連成立以前の受注は帰属記録が無いので見えない）。事後申領の証明は、完了時に会員へ帰属しなかった受注（顧客の有無を問わない）へ発行されるワンタイムの伝票トークン（QR）の所持のみで、申領期限は完了から 90 日（訂正による再発行のみ例外で、再発行から 90 日）。事後申領は店舗台帳（Customer 行・行級の関連）に一切波及しない。関連の成立が過去受注の遡及帰属を引き起こすことはない（ADR 0008）。唯一の訂正操作は理由付きの**無効化**で、ポイントへは自動波及しない（清算は手動調整で行う）。無効化された受注は会員へ帰属しない状態に戻り、トークン再発行による事後申領の対象へ復帰する。
