...
// central ドメインの既定値（compose の環境変数 CENTRAL_URL で上書き可能）。
// tenant/central の判別は Host で行われる（tenantResolver.ts）ため、baseURL（store1）とは別ドメイン。
export const CENTRAL_URL = process.env.CENTRAL_URL ?? 'http://kizuna.test';
