...
/**
 * トークン単位の Redis ブラックリスト（{@link TokenBlacklistService}、ログアウト用）を検証する {@code
 * OAuth2TokenValidator}。判定は毎回 Redis へ問い合わせる必要があるため、{@link JwtDecoderConfig} が組み立てる decoder
 * にはキャッシュを付けないこと（付けるとブラックリストと版照合の即時性が壊れる）。
 *
 * <p>失敗時の {@link OAuth2Error} には内部理由を含めない。 Redis 接続断で例外が投げられた場合はこの検証を貫通して呼び出し元へ伝播し、リクエストは 500
 * になる（fail-closed）。
 */
@Component
