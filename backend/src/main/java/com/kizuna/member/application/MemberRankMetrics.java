 ...
 *
 * <p>材料そのものは order と point が持ち member からは引けない（依存が環になる）ため、実装は記帳する側に置く。値ではなくこの型で受けるのは、 <b>読む時点</b>を
 * member 側が決めなければならないため — ロックの前に読んだ値で判じると、並行する 2 つの付与が同じ古い指標を観測し、 どちらも昇格させないまま閾値を跨いだ来店が取り残される。
 */
