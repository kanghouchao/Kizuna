 ...
 *
 * <p>顧客参照を書くのは 4 経路 — 受注録入の顧客 ID 指定・受注録入の電話照合で 1 件一致した行・会員申請の確定時自動整備・会員紐づけの成立。
 * 既にある行を指す参照はどれもここを通ることで「書く直前に対象の顧客行を悲観排他ロックする」規律を共有する。経路ごとに解決を持つ形だと、 5 つ目の経路が増えたときに直列化の抜けが静かに再び開く。
 *
