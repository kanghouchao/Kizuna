 ...
 *
 * <p>挿入のみで削除はしない。既定ロールの権限を減らす／権限コードを廃止するのはいずれも既存の授与を失わせる判断であり、明示的な移行 changeset に属する（{@code
 * fk_t_role_permissions_permission} は RESTRICT で、参照中の目録行の削除を拒否する）。
 *
...
public class PermissionCatalogueSeed implements CustomTaskChange {
  ...
  public String getConfirmationMessage() {
    return "権限目録 %d 行・既定ロールへの授与 %d 行を追加しました".formatted(insertedPermissions, insertedGrants);
  }
