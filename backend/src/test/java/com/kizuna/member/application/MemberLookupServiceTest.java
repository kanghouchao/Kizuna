...
class MemberLookupServiceTest {
  ...
  void findByMemberCodeReturnsMinimalLookup() {
    Mockito.when(memberRepository.findIdentityByMemberCode("123456789012"))
        .thenReturn(Optional.of(identity(4L, "123456789012")));

    Optional<MemberLookup> lookup = service.findByMemberCode("123456789012");

    assertThat(lookup).isPresent();
    assertThat(lookup.get().memberId()).isEqualTo(4L);
  ...
  void findByMemberCodeReturnsEmptyWhenUnknown() {
    Mockito.when(memberRepository.findIdentityByMemberCode("999999999999"))
        .thenReturn(Optional.empty());

    assertThat(service.findByMemberCode("999999999999")).isEmpty();
  }

  private static MemberIdentityView identity(Long id, String memberCode) {
