 ...
 * を挿入し IDENTITY シーケンスが未進行）がそのまま成立する。changeset で setval 整合されていれば、中央 API からの初回店舗作成が リトライ不要で成功し、3
...
class SeedSequenceAlignmentIT {
  ...
  @Test
  @DisplayName("全新 DB で中央 API から初回店舗作成がリトライ不要で 204 になること")
  void firstStoreCreationSucceedsOnFreshDb() {
    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.APPLICATION_JSON);
