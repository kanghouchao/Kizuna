...
- **Disabling a Steiger rule requires a reason comment in steiger.config.mjs** (typo-in-layer-name / insignificant-slice are off by design decision or rule specification; excessive-slicing is off for `_pages/**` only — one screen = one slice there).
