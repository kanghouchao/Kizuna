...
│                 #   Split into two root layouts: `(admin)` (console + cast portal, owns the theme wiring)
...
- **Store context is a single seam**: `entities/user`'s `StoreContextProvider` / `useStoreContext` (mounted in both console layouts, `src/app/(admin)/platform/layout.tsx` and `src/app/(admin)/store/layout.tsx`) resolves the _current user's switchable stores_ **once** — the `storeBridge` qualification is read synchronously from the token claims (`readTokenClaims()` in `shared/lib`), then `stores()` is fetched only when it is true (otherwise the list is empty by design). Display/routing data that lives in the JWT claims (authorities / userType / storeBridge) is read via `readTokenClaims()`, never re-fetched from `/platform/me`; `me()` remains for data outside the token (`console` at login, `display_name` on the header and account pages). Consume that through `useStoreContext()`; do not re-fetch it per component. The administrative **catalog** is a different question and stays a direct `platformAuthApi.stores()` call (fetched once by staff management's list page and shared with its modals/pickers via props), because an HQ admin with `store_bridge=false` has an empty switchable list yet still assigns every store.
