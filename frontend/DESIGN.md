...
- The toasts (`ToastProvider`) are three rows already in the table, none of them new: the base is `text-foreground` on `bg-card` (`--card-foreground` is defined identically to `--foreground` in both modes, so it is literally that row), the success toast is `bg-success` + `text-success-foreground`, and the error toast is `bg-destructive` + `text-destructive-foreground`. react-hot-toast paints its own background from a generated class, and a Tailwind token class layered on top wins or loses by source order — so each pairing is written as an inline `var(--token)` instead, which is deterministic and follows the mode the same way. Two consequences bind anyone editing that file: a per-type `style` is **merged over** the base one, so overriding a background without also naming its `color` leaves the base foreground on the new fill; and the toast's drop shadow lives on the generated class, so an inline `background` never removes it.
  ...
  As the classification stands, none of the three tiers reaches for the base pairing — `success`, `error` and `warning` all paint a hue — and the two `warning` call sites (`StaffEditModal` and `RoleFormModal`) were traced to their host routes, both under `app/(admin)/`. So no white card is printed today. The `blank` per-type key is also left empty **on purpose**, precisely so this warning keeps its force: parking a tier there would make the base pairing unreachable in practice and quietly turn every typeless toast into that tier. None of this is a retraction of the warning above — it is a note on the current state so the next reader does not re-count the tiers.

  The tiers add no row to this table, and this is why: each one reuses a pairing already listed. `error` is this row; `warning` is `bg-warning` + `text-warning-foreground`; every tier's icon is `currentColor`, so it resolves to the foreground of its own row. The two `iconTheme` corrections exist to pull the library's hardcoded icon colours (`#fff` on red at 2.89 in dark, `#fff` on `#61d345` at 1.92) onto those same tokens — a dependency's default is not an exemption from the rule below. Note also that **`warning`'s pairing is not written in `ToastProvider`** like the other two: that tier has no per-type key, so the semantic layer carries its `style`. Finally, the headroom on this row: light is **4.56** against the 4.5 text bar, 0.06 to spare. Nothing in the tier system created that, but anything layered onto this row is working with almost none. Note the headroom while reading this row, though: light is **4.56** against the 4.5 text bar, 0.06 to spare. Nothing in the tier system created that, but anything layered onto this row is working with almost none.
...
**Two tiers live in the provider and one lives in the layer, and that split is deliberate.** `success` and `error` have per-type keys in the library, so their durations, colours and icon repairs are entries in `ToastProvider`. `warning` has no key — the keys are `success` / `error` / `loading` / `blank` / `custom` — so **the semantic layer owns all of it**: duration, icon, colours, and the prefix below. **The word `warning` does not appear in `ToastProvider` at all**; do not go looking for it there.

What the layer emits for that tier is a **bare `toast()` carrying its own `style`**. Two library facts make that work and one makes it necessary:

- `style` is merged three deep — base, then per-type, then the call's own — so passing `background` and `color` alone keeps the base `borderRadius`. Everything else merges at the options level, the call last.
- A bare toast's type is `blank`, whose default icon is `null`, so the triangle we hand in is the only icon and there is nothing to suppress.
- The duration comes from the call **because of the `blank` decision below and for no deeper reason.** The store resolves it as call → per-type → base → the library's own per-type default, so a provider-side duration does take effect — that is exactly how `success` and `error` get theirs. What genuinely does not work is swapping a duration inside the `ToastBar` children render prop: dismissal is scheduled from the already-merged value before that runs.

The `blank` per-type key could hold this tier's whole appearance instead, putting all three tiers' colours in one file. **Do not.** `blank` is shared with every bare `toast()`, so the tier's look would be inherited by any toast written without a type — and a stray toast painting a white card is merely ugly, while one painting yellow with a stop-triangle and ten seconds is a **lie about severity**. Severity must never be something a toast acquires by accident. Splitting the tier the other way (icon in the provider, duration at the call site) is worse than either: the definition would break across two files, joined only by a marker string that degrades silently to a plain `error` if it is ever misspelled.

#### `warning` differs from `error` by colour and shape

`warning` takes `bg-warning` + `text-warning-foreground` — **already a matrix row** (6.23 / 11.58) — so **no new state semantic and no new matrix row**, and no token is added. Its icon is `icon: <TriangleAlertIcon size={20} className="shrink-0" />`, handed in by the semantic layer:
...
- **`shrink-0` is not optional**, and the reason is a three-way branch worth knowing before anyone "tidies" it away. The library wraps its **own default** icons in a box carrying `min-width: 20px` / `min-height: 20px`, and wraps a **string** icon in a second wrapper that also carries `min-width: 20px` — but an icon passed as a **React element is returned as-is** and lands directly in the toast's own flex row, beside a message that is `flex: 1 1 auto`. There it inherits `flex-shrink: 1` / `min-width: auto` and nothing holds its width. Measured — with the octagon this tier first used, in this same row — it crushes to **10.7×20** and the glyph becomes a vertical ellipse; the mechanism is the row, not the glyph, so the triangle crushes identically. **It appears only when both modes are painted in a real browser**: neither static review nor jsdom sees it. Treat the class as part of the icon's identity rather than as styling, and do not "clean it up".

`error`'s own icon is **corrected**, not restyled. The library's default paints a hardcoded circle-and-cross onto our red fill, and that cross measures **2.89 in dark** against `--destructive` — under the 3:1 graphic bar, an unmeasured pairing that arrived through a dependency rather than through this document. So the `error` per-type block sets `iconTheme: { primary: 'transparent', secondary: 'var(--destructive-foreground)' }`: the circle was effectively invisible anyway (1.44 / 1.14) and is made explicitly transparent, while the cross moves onto the same token as the body text (**2.89 → 6.88**). **The repair deliberately does not change the shape** — an opaque white circle would give `error` a crisp round symbol it does not otherwise have.

`success`'s default icon gets the same treatment for the same reason: the library paints a white check on its own hardcoded `#61d345`, **1.92** against that green, and neither colour is in this document. The `success` per-type block therefore sets `iconTheme: { primary: 'transparent', secondary: 'var(--success-foreground)' }`, putting the check on the token its own row already certifies (`bg-success` + `text-success-foreground`). Both repairs exist because a colour pairing must be one this document measured — a dependency's default is not an exemption from that.
...
- **`role="status"` / `aria-live="polite"` and `top-center` are the library defaults and stay that way** — for `error` and `warning` too, since the package ships no assertive variant at all. There is no per-tier override to write, at the provider or at the call site. A tier is never escalated to `assertive`: interrupting a reader mid-utterance is not how severity is conveyed here.
...
- One limit those two leave standing, named here rather than implied away: **a reader who needs longer than the duration cannot keep the toast.** Hovering pauses the timer (the container wires the library's pause handlers to the pointer), but that is pointer-only again and so does not answer the keyboard case; and both obvious repairs are closed off, since a persistent toast was rejected together with the × button and nothing interactive may go in the body. The reading to carry forward is therefore that **a toast is a poor sole record** — where a failure leaves lasting state, the durable answer is for the screen holding that state to say so, which is clause ①'s logic applied to a modal whose values were just replaced. Growing the timer is not that answer.
...
- The shape lives in **one hand-written component in `shared/ui`** (the side without `data-slot`) with three **semantic** props — the copy, an optional retry handler, and the alternative link for 404 — **plus a `className` passed through**, since placement is the caller's and every hand-written component in that directory already accepts one. Three is the number of decisions the component makes, not the number of attributes it accepts. Retry being optional is how "no retry on a 404" is expressed. Adding to or editing `shared/ui` is a shared-file change and goes through the parallel-PR contract below.
...
- **The browser must not be able to pre-empt the handler**, and the way that is guaranteed is **`noValidate` on the `<form>`**. While any unmet native constraint is live the browser stops the submit before `handleSubmit` runs and our message is never drawn — and the class is wider than it first looks: `required`, `minLength`, `min`, `type="email"`. One attribute on the form settles all of them at once, including the next constraint someone reaches for, instead of requiring the list to be re-inventoried each time.
  ...
  - **That route covers native inputs only.** A Radix-controlled field has no native attribute to keep, because its focusable element is a button — so **pass `required` to the primitive instead**: `Select`, `Checkbox` and `RadioGroup` each emit `aria-required` on their own trigger from it. Do not hand-write the attribute, and do not assume `rules.required` covers it — that configures react-hook-form and never reaches the markup. Without this the group checks that clause ② exists for are exactly the fields that announce nothing about being required until the submit is pressed.
...
**A `register` option that transformed the value has to be reproduced when the field moves.** `{...field}` has no equivalent of `valueAsNumber`, so `field.onChange` receives the number input's string and the payload starts carrying `"3"` where it carried `3`. Restore the type at the `onChange` boundary, exactly as "Type restoration" below prescribes for Radix values — and note that the precedent above only survives because its submit handler coerces with `Number()`. "Behavior preservation" makes the submitted payload the bar, so this is not a detail to discover from a failing backend. Wrap the whole form in `<Form {...form}>`. Never introduce a bare `Controller`.
