...
**One registration path per field.** What moves inside a `FormField` takes `{...field}` from its render prop and **drops its `register` call** — `FormField` _is_ the `Controller`, so keeping both registers the same name twice and leaves the controller and the element contending over ref, rules and reset. Precedent: the 人数 field in `ReservationRequestEditModal`, a native `Input` wired entirely through `field`. Wrap the whole form in `<Form {...form}>`. Never introduce a bare `Controller`.
