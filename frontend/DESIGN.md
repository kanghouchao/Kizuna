...
- The toasts (`ToastProvider`) are three rows already in the table, none of them new: the base is `text-foreground` on `bg-card` (`--card-foreground` is defined identically to `--foreground` in both modes, so it is literally that row), the success toast is `bg-success` + `text-success-foreground`, and the error toast is `bg-destructive` + `text-destructive-foreground`. react-hot-toast paints its own background from a generated class, and a Tailwind token class layered on top wins or loses by source order — so each pairing is written as an inline `var(--token)` instead, which is deterministic and follows the mode the same way. Two consequences bind anyone editing that file: a per-type `style` is **merged over** the base one, so overriding a background without also naming its `color` leaves the base foreground on the new fill; and the toast's drop shadow lives on the generated class, so an inline `background` never removes it.
  ...
  The tiers add no row to this table, and this is why: `critical` reuses the error row's recipe unchanged, its octagon icon is `currentColor` (= `--destructive-foreground`, so the same row again), and the correction to `error`'s default icon colours moves that cross onto the same token. No pairing outside the rows above is prescribed anywhere in that section. One dependency-supplied pairing does survive uncorrected, though, and it is recorded rather than implied away: the library's **default `success` icon is a white check on its own hardcoded `#61d345`**, which is **1.92** against that green and is in no row here. It is the same defect the `error` correction repaired and the same `iconTheme` repair would land it on the `bg-success` + `text-success-foreground` row — but only `error` was measured and ruled on, so the success icon stays as the dependency ships it until someone rules otherwise. Note the headroom while reading this row, though: light is **4.56** against the 4.5 text bar, 0.06 to spare. Nothing in the tier system created that, but anything layered onto this row is working with almost none.
...
- Three consequences, all accepted deliberately. Dismissal is **pointer-only**, which is tolerable **because all three durations are finite** — pairing body-click dismissal with a persistent toast would produce a banner a keyboard user could never close. **Nothing interactive can live inside a toast**, because the whole body is the dismiss target; recovery affordances belong in the failing region, which is where clause ① already puts them. And since the octagon is the whole visible difference, **the tier distinction is visual only** — assistive technology is told the same thing for `error` and `critical`, so `critical`'s wording has to carry its own weight rather than leaning on the icon.
...
- **The region has to announce the change, not merely be drawn.** The failure arrives after the region has mounted, and clause ① has already ruled out the toast, so a reader who is not watching that part of the screen gets nothing: the rows they were working through vanish and a retry button they cannot see appears. This is the same standard ② is held to — being beside the input is not enough if nothing is announced. **Which mechanism carries it is not settled** (a live region on the shared component, or moving focus to the retry button, which is stronger but takes focus away from wherever the reader was), so raise it rather than picking one per call site.
...
- **The browser must not be able to pre-empt the handler.** While a native constraint is present and unmet, the browser stops the submit before `handleSubmit` runs, so our message is never drawn: the two are alternatives, not partners. For **`required` and `minLength` the resolution is settled — take the attribute off** — and removing one is only safe together with the copy that replaces it, otherwise the field falls silent instead of speaking Japanese.
  - Two consequences to carry with that. A removed `required` also removes the field's **required state** from assistive technology: `FormControl` emits `aria-invalid` and `aria-describedby` but **not** `aria-required`, so a required field marks itself. And the class is wider than the two attributes named above — a `min` on a number input pre-empts the handler in exactly the same way. **For a constraint attribute that also carries non-validation value (`type="email"` earns the right keyboard, `min` gives a spinner its floor) the mechanism is not settled**: removal costs something real there, and suppressing native validation form-wide instead is a decision nobody has taken. Raise it rather than choosing per file.
