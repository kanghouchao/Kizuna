...
| `bg-success/10` hover over `bg-muted` + `text-foreground`        | 16.23     | 11.75     | 4.5  |
...
**Recorded deviation — the shift timeline's in-bar buttons.** The two buttons inside a timeline bar (edit, and the publication eye) keep the browser's default outline and hand-write no ring. Both of this rule's escapes are unavailable there: the ring's ground is the bar's own fill (`bg-success`, unmeasured) or the `bg-muted` track, where `ring-primary` measures **2.83 in dark** and fails; and the bar's `overflow-hidden` clips an outer ring, while `ring-inset` would sit on those same grounds. Moving the ring to a certified surface would mean moving the control out of the bar, which is the placement #722 fixed. A ring recipe that clears 3:1 on both grounds would retire this deviation.
