...
- **Every toast carries a dismiss button, at every tier**, and it is the same control everywhere; it carries no part of the tier distinction. The button is a bare × in `currentColor`, which keeps it on the tier's own certified pairing. The shared ghost `Button` is deliberately **not** used for it: that variant's hover paints `bg-accent`, an admin surface this table never measured against a coloured toast fill.
  ...
  - **Its focus ring is `currentColor`, not `--ring`**, and this is the one place in the console where that token is the wrong answer. `--ring` is drawn for the admin surfaces — it manages 2.62 / 3.67 on `bg-card` — but a toast's ground is a saturated tier fill, where the **opaque** token measures **1.22 / 2.80** (`warning`), **1.82 / 1.67** (`error`), **1.23 / 2.71** (`success`), and the usual `/50` takes all six down to 1.04–1.67. A focus indicator nobody can see undoes the keyboard route entirely: <kbd>F6</kbd> would deliver focus to a control that gives no sign of having it. `currentColor` puts the ring on the tier's own certified row.
