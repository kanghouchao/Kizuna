...
export function ServiceIdentityEditModal({
  ...
  const form = useForm<ServiceIdentityEditFormValues>({ defaultValues: toFormValues(identity) });
  const {
    control,
    handleSubmit,
    reset,
  ...
  // 409 の再取得で identity が差し替わったら、ローカル編集は捨てて最新値で組み直す
  useEffect(() => {
  ...
  const submit = async (values: ServiceIdentityEditFormValues) => {
    ...
    try {
      ...
      if (isConflict(error)) {
        ...
        // 最新値へ自動リフレッシュする（identity prop の更新で上の reset が走る。ローカル編集は破棄、モーダルは開いたまま）。
