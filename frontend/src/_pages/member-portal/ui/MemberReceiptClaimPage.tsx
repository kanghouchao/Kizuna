...
type ClaimFailure = { message: string; retryable: boolean };
  ...
  const claim = useCallback(async (raw: string) => {
    ...
    try {
      ...
      setFailure({
        ...
        message: getApiErrorMessage(error, '取り込めませんでした。通信の状態をご確認ください。'),
        ...
        <CardContent>
          {failure !== null ? (
            failure.retryable && token !== null ? (
              ...
              // 逃げ道はホームで、伝票そのものの取り直しは店舗に尋ねるしかない
              <RegionError
                message={failure.message}
                fallback={{ href: '/member/', label: 'ホームへ' }}
              />
            )
          ) : grantedPoints === null ? (
