 ...
 *
 * proxy（middleware）が設定した x-mw-role cookie で tenant ドメインのみ許可し、
 * x-mw-tenant-template cookie の模版キーで templates/<key>/<page> を dispatch する。
 * app 配下の各公開ルートはこの殻を呼ぶだけの薄殻にする。
 */
export default async function StoreSitePage({ page, pageProps = {} }: StoreSitePageProps) {
  const cookieStore = await cookies();
  if (cookieStore.get('x-mw-role')?.value !== 'tenant') {
    notFound();
  }
