...
export function AttendanceFormModal({
  ...
  useEffect(() => {
    ...
    // 予定の開始は後端が暦日へ写した値を使う。勤務日は営業日なので、日付変更時刻より前に始まる枠を
    // 勤務日と時刻から組むと 24 時間ずれる。飛び込みは予定を持たないので表示中の営業日に載せる —
    // 現在時刻だけで起こすと、別の日を見ている操作が黙って今日の営業日へ落ちる。
    reset({
      cast_id: target.shift?.cast_id ?? '',
      business_date: '',
      actual_start_at: target.shift
        ? toDateTimeInput(target.shift.scheduled_start_at)
  ...
  return (
    <ShiftDialogShell
      open={open}
      onClose={onClose}
      title={correcting ? '当日実績を訂正' : '当日実績を記録'}
    >
      <Form {...form}>
        ...
        <form onSubmit={handleSubmit(submit)} className="space-y-4 px-6 py-5" noValidate>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {shift
                ? `${dateOfDateTime(shift.scheduled_start_at)} ${hhmmOfDateTime(
                    shift.scheduled_start_at
                  )}–${hhmmOfDateTime(shift.scheduled_end_at)}`
                : '飛び込み（予定なし）'}
            </p>
          </div>
