...
import { clearPlatformSession, startPlatformSession } from '@/shared/lib';
...
describe('招待発行ボタンの表示は店長ロール限定（#327、codex指摘対応）', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('STORE_MANAGER セッションでは行内に「招待を発行」ボタンが表示されること', async () => {
    startPlatformSession('STORE_MANAGER');
  ...
  it('STORE_STAFF セッションでは「招待を発行」ボタンが表示されず、招待状態バッジは表示されること', async () => {
    startPlatformSession('STORE_STAFF');
...
describe('カスタムフィールド管理への入口リンクは店長ロール限定（#277）', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockedCastApi.list.mockResolvedValue(toPage([cast]));
  });
  ...
  it('STORE_MANAGER セッションでは定義管理ページ(/tenant/casts/fields)への入口リンクが表示されること', async () => {
    startPlatformSession('STORE_MANAGER');

    render(<CastListPage />);
    await screen.findByText('花子');

    const link = screen.getByRole('link', { name: 'カスタムフィールド管理' });
    expect(link).toHaveAttribute('href', '/tenant/casts/fields');
  });

  it('STORE_STAFF セッションでは定義管理ページへの入口リンクが表示されないこと（定義CRUDは店長限定のため）', async () => {
    startPlatformSession('STORE_STAFF');
