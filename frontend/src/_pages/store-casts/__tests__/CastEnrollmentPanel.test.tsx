...
it('履歴を追加読み込みし、失敗時はその領域から再試行できる', async () => {
  ...
  fireEvent.click(screen.getByRole('heading', { name: '在籍履歴', level: 2 }).closest('summary')!);
  fireEvent.click(
    screen.getByRole('heading', { name: '内部情報の編集履歴', level: 2 }).closest('summary')!
  );
...
it('履歴は初期状態で折り畳み、開くと読める', async () => {
  render(<CastEnrollmentPanel cast={{ id: 'e1', status: 'ENROLLED' }} onChanged={jest.fn()} />);
  const summary = screen.getByRole('heading', { name: '在籍履歴', level: 2 }).closest('summary')!;
  expect(summary.closest('details')).not.toHaveAttribute('open');
  expect(
    screen.getByRole('heading', { name: '内部情報の編集履歴', level: 2 }).closest('details')
