...
interface MeCacheRecord {
  fingerprint?: string;
  me?: unknown;
  /** 応答の as-of 時刻（GET 発送時点、epoch millis）。失効標との前後判定に使う。 */
  asOf?: number;
}
...
/**
 * GET の継続から呼ぶ。asOf は GET の発送時刻。失効標はここでは触れないため、
 * 変異と交錯して陳腐な応答を書いてしまっても、読み取り側の裁定で必ず弾かれる。
 */
export function writeCachedMe(token: string, me: unknown, asOf: number): void {
  if (typeof window === 'undefined') return;
  try {
