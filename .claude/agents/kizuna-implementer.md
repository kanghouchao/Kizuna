...
Your prompt gives an **absolute worktree path** and an **absolute plan path**. Before the first change, verify `pwd` is the worktree and `git rev-parse --abbrev-ref HEAD` is the plan's branch (the orchestrator created it; `.env` is deny-ruled for agents — never read or copy it; when a task legitimately needs the stack, `task up`'s `ensure-env` dep auto-provisions it from the main checkout). Never touch the main checkout. **Repo-root files are the classic leak**: CONTEXT.md, README.md, and docs/ exist in the worktree too — build every edit path by prefixing the assigned worktree path, never from memory of the repo layout (a bare `<repo>/CONTEXT.md` silently edits the main checkout; run #322 did exactly this and it surfaced only when the post-merge pull collided). Before reporting, run `git -C <main checkout> status --porcelain` (strip `/.worktrees/<branch>` from your worktree path): if a file YOUR tasks touched appears there, you leaked an edit — do not fix the main checkout yourself; state the leak prominently in your report.
...
- Run every gate in the FOREGROUND with generous timeouts (up to 600000 ms). A stopped agent cannot observe results — never `run_in_background` a gate, and never end your turn while one runs. When a combined gate could exceed the 600s cap (full `task test`), run its components as separate foreground calls instead (`task test-unit`, then `task test-integration`); each fits the cap on its own, and the pair both exiting 0 IS the `task test` gate — report both exit codes as its evidence.
