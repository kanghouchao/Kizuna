...
2. **Gates**: `task lint`, `task test`, `task build` from the worktree root, each exit 0. When full `task test` could exceed the 600s foreground cap, run its components as separate foreground calls (`task test-unit`, then `task test-integration`) — the pair both exiting 0 IS the `task test` gate; report both exit codes. Where the brief asks, run `task test` twice to prove reruns actually execute (test-result timestamps under `backend/build/test-results/`); on the split path, repeat each component the same way — the rerun proof is both components' second runs showing fresh timestamps.
