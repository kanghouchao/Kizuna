...
**Concurrency seam** — if the plan touches shared mutable state reachable by two or more concurrent actors (a status field two requests can race to flip, a collection two requests can each read-modify-write, an "invalidate then insert" sequence), the TDD seams must name it explicitly and pin the concurrency-safety mechanism (conditional update, lock, or equivalent) as its own task — don't leave it to be discovered one race at a time in post-push review (#327: 4 of 13 post-push findings were race variants of the same undesigned seam).

**Capability gating check** — if the plan gates UI visibility or navigation on "does the user hold capability X", verify X's Console classification (PLATFORM/STORE/SHARED) before wiring the condition: a SHARED capability (usable by both consoles) proves the endpoint it guards is reachable, never that the user holds a console-exclusive capability — gating on "the endpoint returned data" instead of the actual capability set silently admits users who cannot act on what they're shown. Cross-check against the exclusive-capability set the backend already uses for its own console-entry decision (e.g. `hasStoreConsole()`), don't re-derive it from endpoint reachability — and once the gate is right, put it in one shared resolver rather than one inline check per screen (#413: the same gap recurred independently in a second screen across post-push review rounds before being unified).
...
- **Every run**: before pushing, run the local review. **Standing form: the two-axis `/code-review` skill (`mattpocock-skills:code-review`, Standards + Spec in parallel sub-agents), run on the plan's 検証モデル** — Stage 2 owns the model routing here as everywhere (the three adoption-evidence runs — #405/PR#407: 3件, #409/PR#411: 7件, #415/PR#416: 9件, findings all true — ran it on `fable` per their plans; a normal-risk plan's cheaper 検証モデル applies just as well). The orchestrator invokes it with the absolute worktree path, the fixed point (`master`), and the spec source (issue ref or pasted acceptance criteria — NEVER the plan document; the axis sub-agents have fresh eyes by construction). The two axes cover Standards and Spec only, so the adversarial defect-hunt and 並行性/atomicity lenses from `kizuna-reviewer.md` stay mandatory on EVERY run: the axis briefs must carry them explicitly, weighted by risk tier — normal-risk names the lens (races, dead authorized routes, fail-open paths); high-risk spells out the full hunt (races, fail-open paths, tenant-isolation leaks, permission-preservation on moved code — #415's spec-axis brief is the pattern). **Fallback** when that plugin is not installed in the operator's environment: spawn the committed **`kizuna-reviewer`** agent (`.claude/agents/kizuna-reviewer.md`) — a FRESH spawn on the run's 検証モデル (never a fork: a fork inherits this session's plan context and defeats the fresh-eyes design) with the same brief plus the risk tier; the committed agent exists precisely because bare plugin names are operator-dependent (#346/#347). Whichever form runs, the orchestrator classifies every finding as blocking (correctness・security・data loss・spec violation) or non-blocking (judgement/style) — the same criteria watch-pr's triage uses. Blocking findings → fix loop (shared budget), fixed in the worktree before pushing; re-review after a fix goes to the SAME reviewer (the same axis sub-agents / the same agent, via SendMessage delta brief — like verifier re-runs). Non-blocking findings are recorded for the PR's 備考. The classification's totals feed the PR template's ローカル code-review 実施 line (`effort: medium`（normal-risk）/ `high`（high-risk）, `指摘: n件 / 未修正: n件`). This replaces the former high-risk-only adversarial pre-review.
