...
- **Every run**: before pushing, invoke a local code review against the worktree diff — **`code-review`** (Skill tool, effort `medium` for normal-risk / `high` for high-risk) is the default target. Bare `code-review` can resolve to a different plugin's same-named skill depending on the operator's environment — this repo has no committed marketplace/plugin config (`.claude/settings.json` carries none), so resolution is whatever the operator has installed. Confirmed in run #346/#347: it resolved to **`mattpocock-skills:code-review`**, a two-axis Standards/Spec review with no effort dial, run against `master` (or the PR base) as the fixed point — already an established dependency of this project via other skills (`tdd`, `grilling`, `domain-modeling`). Either resolution is an acceptable substitute for the other; note in the PR's 検証 line which one actually ran. This replaces the former high-risk-only adversarial pre-review. Blocking findings → fix loop (shared budget), fixed in the worktree before pushing. Non-blocking findings are recorded for the PR's 備考 (effort level, or `N/A（two-axis）` for the two-axis form; finding count; unresolved count feed the PR template's ローカル code-review 実施 line).
