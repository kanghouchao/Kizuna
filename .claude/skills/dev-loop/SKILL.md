...
- **Every run**: before pushing, run the local review. **Standing form: the two-axis `/code-review` skill (`mattpocock-skills:code-review`, Standards + Spec in parallel sub-agents) on `fable`** — adopted after three consecutive runs with findings all true (#405/PR#407: 3件, #409/PR#411: 7件, #415/PR#416: 9件). The orchestrator invokes it with the absolute worktree path, the fixed point (`master`), and the spec source (issue ref or pasted acceptance criteria — NEVER the plan document; the axis sub-agents have fresh eyes by construction). **Fallback** when that plugin is not installed in the operator's environment: spawn the committed **`kizuna-reviewer`** agent (`.claude/agents/kizuna-reviewer.md`) — a FRESH spawn on the run's 検証モデル (never a fork: a fork inherits this session's plan context and defeats the fresh-eyes design) with the same brief plus the risk tier; the committed agent exists precisely because bare plugin names are operator-dependent (#346/#347). Blocking findings → fix loop (shared budget), fixed in the worktree before pushing; re-review after a fix goes to the SAME reviewer (the same axis sub-agents / the same agent, via SendMessage delta brief — like verifier re-runs). Non-blocking findings are recorded for the PR's 備考. The report's totals feed the PR template's ローカル code-review 実施 line (`effort: medium`（normal-risk）/ `high`（high-risk）, `指摘: n件 / 未修正: n件`). This replaces the former high-risk-only adversarial pre-review.
