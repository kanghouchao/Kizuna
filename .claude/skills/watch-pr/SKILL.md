...
- **Green**: CI green AND **every review thread is dispositioned** — bot-resolved (fix confirmed by re-review) or carrying a disposition reply (unresolved, left for the human) — AND every anchorless summary finding triaged. Before reporting, check whether the codex bot has reviewed the **head commit**: take the newest review **authored by the codex bot** (an unfiltered newest-review node may be a human's and says nothing about the bot) and compare its full `commit.oid` against the PR's full `headRefOid` — same SHA form on both sides; `abbreviatedOid` vs a full SHA never matches and reads as a permanent false negative. Zero unresolved threads proves nothing about commits the bot hasn't seen; it reviews each push with a ~10–15 min lag and no delivery guarantee. An unreviewed head still exits green, but the report must say so — merging then means accepting a possible post-merge wave (#376: the wave landed 15 minutes before merge, was missed, and cost follow-up PR #377). → report: checks state, threads resolved vs carried (with their issue refs), commits pushed during the watch, and the head-commit review status.
