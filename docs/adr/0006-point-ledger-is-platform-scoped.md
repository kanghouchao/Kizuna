...
会員側（ポータル）には本人の残高と全種別明細の読み口がある（`/platform/me/points`）。台帳が店舗で
分割されない以上、本人の会員 ID の一致だけが隔離境界であり、問い合わせ自体に載せる。
`MemberFacingLedgerLeakIT` は残高項目をこの端点にだけ許し、他の会員側端点と、顧客台帳の内部項目
（店員が書く手動調整の理由を含む）は端点を問わず拒否し続ける。
