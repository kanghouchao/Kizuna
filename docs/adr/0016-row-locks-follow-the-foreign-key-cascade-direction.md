...
指名を書き込む受注作成・指名差し替え・申請確定と会員申請の受付は、店舗の FOR KEY SHARE → 対象店舗の在籍の FOR UPDATE → 顧客参照の解決 → 受注・申請の書き込みの順を守る。在籍状態はロック後に検証し、取引の確定まで保持することで停止・退店との競合を直列化する。候補一覧はロックせず、新しい指名を立てない既存受注の据え置き編集にはこの検証を課さない。
