...
- **范围**：13 对 FooService/FooServiceImpl 中，11 对只有一个 adapter 且无任何测试以 interface 为 seam —— Impl 并入类名（`CustomerServiceImpl` → `CustomerService` 类）。
...
- **同步**：`.claude/rules/backend.md` 模块模板更新（application 层不再默认 interface+Impl；出现第二 adapter 或跨模块 mock 需求时再引入 interface）。
- **验收**：全测试绿、ModularityTests 绿、无行为变化；删除约 11 个文件。
...
- **范围**：settings 模块导出 `SmtpSettings` record（键名知识归 settings interface 所有），`smtpSettings()` 加入 SystemConfigService interface；MailServiceImpl 删除 5 处裸字符串键消费类型；顺手处理 `// ponytail:` 注记的 JavaMailSender 每次重建（简单失效缓存：配置更新事件或 TTL，实现自选最小方案）。
...
- `TenantIsolationTests` 扫描依据从「TenantScopedEntity 子类」改为「**所有映射了 tenant_id 列的 @Entity**」，断言各自声明 @Filter(tenantFilter)。
- `StoreProfile`（手抄 tenant_id/时间戳/@PrePersist）迁移为继承 TenantScopedEntity；`StoreMenu` 同样归位并挂 @Filter，`StoreMenuServiceImpl` 的手写 findByTenantId 过滤改走统一机制（保留显式查询与 filter 双保险亦可，但必须进入机器检查覆盖）。
- **验收**：机器检查覆盖 8/8 租户实体；跨租户隔离 E2E（用 X-Tenant-ID: 1 建数据、X-Tenant-ID: 2 查不到）。

### Step 2（需 spike 验证可行性）：filter 启用从 24 处方法注解上移到请求 seam

- 设想：TenantIdInterceptor 解析出租户后即对本请求的 Hibernate Session 启用 filter，@TenantScoped 仅保留给非请求上下文（事件监听、定时任务）作逃生门。
- **未决点（实现前 spike）**：Hibernate filter 是 Session 级——请求早期 Session 是否已可得取决于 OSIV 配置；若 OSIV 关闭则需 TransactionSynchronization / SessionEventListener 方案。**若 spike 结论是复杂度不划算，只交付 Step 1，Step 2 记 ADR 放弃。**
...
- C2 Step 2 的 spike 结论（采纳或 ADR 放弃）
