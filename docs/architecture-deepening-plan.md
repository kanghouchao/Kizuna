...
- **PR 评审中发现并修复（Critical）**：`TenantScopedEntity` 的 `@FilterDef(tenantFilter)` 未设置 `applyToLoadByKey`（Hibernate 默认 `false`），导致 `Session#find`（Spring Data JPA 的 `findById` 走此路径）不受 filter 约束——`CustomerService.get/update`、`CastService.get/update`、`OrderService.update` 均直接 `findById(id)` 无额外校验，跨租户指定他人 ID 即可直接读取/篡改数据。`delete()` 系方法（Customer/Cast/Order 各一处）未受影响，但**并非因为 `deleteById` 本身受 filter 约束**——`deleteById` 的默认实现是 `findById(id).ifPresent(this::delete)`，与 `findById` 走同一条不受 filter 约束的 `Session#find` 路径；三处 `delete()` 之所以安全，是因为各自在调用 `deleteById` 前先有一道 `existsById(id)` 守卫（`existsById` 走 JPQL count 查询、真正受 filter 约束），不存在则先抛异常。这意味着若未来新增 `delete()` 方法却省略这道 `existsById` 前置检查，会重新打开同一个跨租户漏洞——此约束目前只靠人工审查维持，机器检查未覆盖。修复：`applyToLoadByKey = true` 补上；`TenantIsolationTests` 新增反射断言验证该开关恒为 true；手动 curl 复验（tenant 1 建 Customer → tenant 2 用同 ID GET，修复前应 200 泄漏、修复后 400 未找到）确认生效。
...
**PR-B 门禁**：`task lint` + `task test` 全绿（含 applyToLoadByKey 反射断言）；跨租户负向验证以手动 curl 完成——本仓库目前无 Postgres 集成测试基建（`task test` 的 Docker build stage 无法访问外部 DB，全仓库尚无一个 `@DataJpaTest`/Testcontainers 用例），自动化跨租户集成测试留作独立议题（需先决定引入 Testcontainers 或让 CI 测试阶段接入 compose 网络）。因动安全语义，评审需独立进行。
