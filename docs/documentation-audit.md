...
PR #897 の 5 件の指摘を確認し、次を修正した。文書に加えて Java の注釈とテスト失敗時の診断文を更新したが、業務ロジック・API・授権判定は変えていない。

| 指摘 | 照合根拠 | 処置 |
| --- | --- | --- |
| 歴史資料の凡例が不足 | 基準コミットの旧資料 3 件の凡例 | 共通索引へ移行元提案・Kizuna 既定・業務確認待ちを含む全分類を復元。確認済みは旧コード上の事実、既定は調査時点の決定であって実装完了ではないと明記 |
| 削除済み API 規範の引用がソースに残る | EndpointAuthorizationDeclarationTests と全ソースの文字列検索 | 同テストおよび別の 5 Java ファイルの注釈を backend/AGENTS.md の各 API 章へ更新。旧章番号をテストの診断文からも除去 |
| ADR 0026 の新しい状態表記が英語 | 人向け文書の言語方針 | 「状態: 採用」に変更 |
| 初期アカウントのパスワード設定方法が欠落 | application.yml のパラメータ、seed の bcrypt 検査、開発 compose の環境変数引継ぎ | README に対話式 htpasswd、管理者・demo 用の 2 ハッシュ、シングルクォート、初回起動前の設定を追加 |
| 帰属無効化とポイント巻き戻しを索引で混同 | ADR 0009・0012・0023 | 帰属無効化、無効化後のポイント訂正、受注のポイント巻き戻しを別々に案内 |

設定例はランダムな一時値で bcrypt 生成・パスワード照合・seed の形式条件・Compose のシングルクォートによる値保持を確認した。Compose が config 出力時に行うドル記号のエスケープを考慮して比較している。実際の開発用 .env は読み書きしていない。

本補正の最終検証は `task lint`（整形後の再実行）、`task test`、`task build`、`task e2e` がすべて exit 0。E2E は全 40 シナリオが成功した。52 Markdown のリンク・アンカー・ローダー検査と `git diff --check` も exit 0。Java 差分は注釈とテスト診断文のみであることをローカルレビューで確認した。
